import type { Bounds } from "./use-measure";
import type { GridSettings, TraverseType } from "./grid-types";
import {
    getPositionForIndex,
...
export function GridContextProvider({
    ...
    const [traverse, setTraverse] = useState<TraverseType | null>(
        null,
    );
    const dropRefs = useRef<Map<string, RegisterOptions>>(new Map());
    ...
    function getRelativePosition(
        ...
        const temp = dropRefs.current.get(targetId);
        if (temp != null) {
            const { left, top } = temp;
            return {
                x: fx - left,
    ...
    function startTraverse(
        ...
        const { x: fx, y: fy } = getFixedPosition(sourceId, x, y);
        const { x: rx, y: ry } = getRelativePosition(
            targetId,
            fx,
            fy,
        );
        const a = dropRefs.current.get(targetId);

        if (a != undefined) {
            const { grid: targetGrid, count } = a;
